...
public interface ClubGalleryRepository extends JpaRepository<ClubGallery, Long>{
	...
	List<ClubGallery> findByCgnameAndClub_cnoContainingOrderByCgnoDesc(String gname, long cno, Pageable pageable);
	
	// 갤러리 검색기능 구현용(제목에 text가 포함된 갤러리 개수)
	long countByCgnameAndClub_cnoContaining(String text, long cno);
}
